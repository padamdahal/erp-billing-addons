...
(function($) {
    ...
    $.extend(NepaliCalendar.prototype, {
        ...
        leapYear: function(a) {
            return this.daysInYear(a) !== this.daysPerYear
        },
        ...
        daysInYear: function(a) {
            ...
            a = b.year();
            if (typeof this.NEPALI_CALENDAR_DATA[a] === 'undefined') {
                return this.daysPerYear
            }
            var c = 0;
            for (var d = this.minMonth; d <= 12; d++) {
                c += this.NEPALI_CALENDAR_DATA[a][d]
            }
            return c
        },
        daysInMonth: function(a, b) {
            ...
            this._validate(a, b, this.minDay, $.calendars.local.invalidMonth);
            return (typeof this.NEPALI_CALENDAR_DATA[a] === 'undefined' ? this.daysPerMonth[b - 1] : this.NEPALI_CALENDAR_DATA[a][b])
        },
        weekDay: function(a, b, c) {
            return this.dayOfWeek(a, b, c) !== 6
        },
        toJD: function(a, b, c) {
            ...
            var g = b;
            var h = a;
            this._createMissingCalendarData(a);
            var i = a - (g > 9 || (g === 9 && c >= this.NEPALI_CALENDAR_DATA[h][0]) ? 56 : 57);
            if (b !== 9) {
                f = c;
                g--
            }
            while (g !== 9) {
                ...
                f += this.NEPALI_CALENDAR_DATA[h][g];
                g--
            }
            if (b === 9) {
                f += c - this.NEPALI_CALENDAR_DATA[h][0];
                if (f <= 0) {
                    f += (e.leapYear(i) ? 366 : 365)
                }
            } else {
                f += this.NEPALI_CALENDAR_DATA[h][9] - this.NEPALI_CALENDAR_DATA[h][0]
            }
            return e.newDate(i, 1, 1).add(f, 'd').toJD()
        },
        fromJD: function(a) {
            var b = $.calendars.instance();
            var c = b.fromJD(a);
            var d = c.year();
            var e = c.dayOfYear();
            var f = d + 56;
            this._createMissingCalendarData(f);
            var g = 9;
            var h = this.NEPALI_CALENDAR_DATA[f][0];
            var i = this.NEPALI_CALENDAR_DATA[f][g] - h + 1;
            while (e > i) {
                g++;
                if (g > 12) {
                    g = 1;
                ...
                i += this.NEPALI_CALENDAR_DATA[f][g]
            }
            var j = this.NEPALI_CALENDAR_DATA[f][g] - (i - e);
            return this.newDate(f, g, j)
        },
        _createMissingCalendarData: function(a) {
            var b = this.daysPerMonth.slice(0);
            b.unshift(17);
            for (var c = (a - 1); c < (a + 2); c++) {
                if (typeof this.NEPALI_CALENDAR_DATA[c] === 'undefined') {
                    this.NEPALI_CALENDAR_DATA[c] = b
                }
            }
        },
