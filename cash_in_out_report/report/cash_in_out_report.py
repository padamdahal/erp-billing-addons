...
class cash_in_out_report(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(cash_in_out_report, self).__init__(cr, uid, name, context=context)
        self.count = 0
        self.total_in_amount = 0
        self.total_out_amount = 0
        self.uid = uid
        self.cr = cr
        ...
        self.localcontext.update({
             ...
             'get_in_amount': self.get_in_amount,
             'get_out_amount': self.get_out_amount,
             'get_total_in_amount': self.get_total_in_amount,
             'get_total_out_amount': self.get_total_out_amount,
             'get_balance_amount': self.get_balance_amount,
    ...
    def get_invoice_ids(self,data):
        ...
        self.cr.execute("select id from account_invoice ai "\
                   "WHERE (ai.date_invoice >= %s) AND (ai.date_invoice <= %s) AND (ai.shop_id = %s) AND (ai.state = %s)", (start_date, end_date, shop_id, 'paid'))
    ...
    def get_in_amount(self,obj):
        if obj.type == 'out_invoice':
            self.total_in_amount += obj.amount_total
    ...
    def get_out_amount(self,obj):
        if obj.type == 'in_invoice':
            self.total_out_amount += obj.amount_total
            return obj.amount_total
        else:
            return '0'
            
    def get_total_in_amount(self):
        return self.total_in_amount
        
    def get_total_out_amount(self):
        return self.total_out_amount
    ...
    def get_amount_in_words(self):
        balance = self.total_in_amount - self.total_out_amount
        amount_in_words = amount_to_text_en.amount_to_text(balance, currency='INR')
...
report_sxw.report_sxw(
    'report.cash.inout.report',
    'account.invoice',
    'cash_in_out_report/report/cash_in_out_report.rml',
    parser=cash_in_out_report,
    header="False"
)
