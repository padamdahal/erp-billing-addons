...
class consumption_report_wizard(osv.osv_memory):
    ...
    def _check_dates(self, cr, uid, ids, context=None):
        for wizard in self.read(cr, uid, ids, ['start_date', 'end_date'], context=context):
            start_date = datetime.strptime(wizard['start_date'], '%Y-%m-%d').date()
            end_date = datetime.strptime(wizard['end_date'], '%Y-%m-%d').date()
    ...
    _constraints = [
        (_check_dates, 'Warning! The difference between two dates is not more than 6 months and not less than 1 month.', ['start_date', 'end_date'])
    ]
    ...
    def get_monthly_product_consumption(self, cr, uid, ids, wizard, month, product, context=None):
        ...
        for invoice in invoice_obj_list:
            ...
            invoice_month = invoice_date.strftime(r"%b-%y")
            if month == invoice_month:
                for line in invoice.invoice_line:
                    if line.product_id and line.product_id == product:
                        monthly_consumption += line.quantity
        return monthly_consumption
    
    def get_consumpted_product(self, cr, uid, ids, wizard, context=None):
        product_data = []
        invoice_ids = self.pool.get('account.invoice').search(cr, uid, [('date_invoice','>=',wizard.start_date),
                  ('date_invoice','<=',wizard.end_date),('shop_id', '=', wizard.shop_id.id),('state','=','paid'),('type','=','out_invoice')])
        invoice_obj_list = []
        ...
        product_data = list(set(product_list))
        return product_data
    ...
    #Method for the Print Consumption Trend Report (Last 3/6 Month) in Excel Format
    def print_excel_report(self, cr, uid, ids, context=None):
        import base64
        wizard = self.browse(cr, uid, ids[0], context=context)
        filename = 'Consumption Report.xls'
        ...
        worksheet.write_merge(3,3,0,6,'Consumption Trend Report (Last 3/6 Month)',center_bold_style)
        ...
        worksheet.write(7,1, 'Item/Drug (Unit)', bold_style)
        
        dates = [wizard.start_date, wizard.end_date]
        start, end = [datetime.strptime(_, "%Y-%m-%d") for _ in dates]
        months_list = OrderedDict(((start + timedelta(_)).strftime(r"%b-%y"), None) for _ in xrange((end - start).days)).keys()
        col = 2
        for month in months_list:
            worksheet.write(6,col, month, center_bold_style)
            col +=1
            
        product_data = self.get_consumpted_product(cr,uid,ids,wizard,context=context)
        ...
        if product_data:
            for product in product_data:
                ...
                inner_col = 2
                for month in months_list:
                    monthly_consumption = self.get_monthly_product_consumption(cr, uid, ids, wizard, month, product, context=context)
                    worksheet.write(row,inner_col, monthly_consumption, right_style)
                    inner_col += 1
                count += 1
                row += 1
