...
class stock_expiry_report(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(stock_expiry_report, self).__init__(cr, uid, name, context=context)
        self.count = 0
        self.uid = uid
        self.cr = cr
        ...
        self.localcontext.update({
            ...
            'get_stock_expiry_records': self.get_stock_expiry_records,
    ...
    def get_stock_expiry_records(self, data):
        stock_production_lot_pool = self.pool.get('stock.production.lot')
        expired_lot_ids = stock_production_lot_pool.search(self.cr,self.uid, [('is_expired', '=', True)], order="life_date desc")
        lot_list = []
        if expired_lot_ids:
    ...
    def get_formated_date(self, expiry_date):
...
report_sxw.report_sxw(
    'report.stock.expiry.report',
    'stock.production.lot',
    'giz_stock_report/report/stock_expiry_report.rml',
    parser=stock_expiry_report,
    header="False"
)
